...
class OllamaChatUI(tk.Tk):
    def __init__(self):
        ...
        # Start Ollama service on launch
        ...
        # Model display names (empty by default - users can add custom names)
        ...
        def do_install():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt > 0:
                        update_progress(f"Retry attempt {attempt + 1}/{max_retries}...")
                    
                    # timeout tuple: (connection timeout, read timeout)
                    with requests.post(url, json=payload, stream=True, timeout=(10, 3600)) as response:
                        response.raise_for_status()
                    ...
                    # If we get here without return, installation succeeded
                    return
                    
                except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                    if attempt < max_retries - 1:
                        import time
                        update_progress(f"Connection issue, waiting {retry_delay}s before retry...")
                        time.sleep(retry_delay)
                        retry_delay *= 2  # Exponential backoff
                    else:
                        update_progress(f"✗ Error after {max_retries} attempts: {str(e)}")
                        self.append_system(f"Failed to install {model_id}: Connection failed after {max_retries} attempts")
                        messagebox.showerror("Installation Failed", 
                            f"Error installing {model_id}:\n{str(e)}\n\n"
...
Be thorough and informative. Synthesize the information rather than just listing it. If the search results don't fully answer the question, clearly state what information is missing."""
    ...
    def on_exit(self):
        """Exit the application and stop Ollama service"""
        try:
            # Stop Ollama service to free GPU resources
            self.append_system("Stopping Ollama service...")
            subprocess.run(['sudo', 'systemctl', 'stop', 'ollama'], 
                         capture_output=True, timeout=5)
            self.append_system("Ollama service stopped. Goodbye!")
            # Give user a moment to see the message
            self.after(1000, self.destroy)
        except subprocess.TimeoutExpired:
            # If stopping takes too long, just exit anyway
            self.destroy()
        except Exception as e:
            # If something goes wrong, still exit
            print(f"Error stopping Ollama: {e}")
            self.destroy()

    def _start_ollama_service(self):
        """Start Ollama service when UI launches"""
        try:
            ...
            # Don't crash the UI if Ollama fails to start
